...
function ssTab(ts,tt){
    $(ts).parent().addClass('active').siblings().removeClass('active');
    $(tt).show().siblings().hide();
}

var switchs = 1;
function chagngeCate(obj,id,attr){
    if(switchs==1) {
    switchs = 0;
    $.get('/changeCate',{id:id},function(data){
        $(attr).html(data);
        $(attr).addClass('in active');
        if($(attr).siblings().hasClass('in')){
            $(attr).siblings().removeClass('in active');
        }
        switchs = 1;
    });
    }
