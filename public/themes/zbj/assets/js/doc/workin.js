    ...
    $('#focus_uid').on('click',function()
    {
        ...
        if(n == '加关注') {
            $.get('/bre/ajaxAdd', {'focus_uid': focus_uid}, function (data) {
                if (data.code == 1) {
                    $("#focus_uid span").attr('class', 'followed-me').html('已关注');
                    $("#focus_uid i").attr('class','fa fa-minus text-size14 cor-grayac');
                }
                if(data.code == 2){
                    alert('请先登录');
                }
        ...
        if(n == "已关注") {
            $.get('/bre/ajaxDel', {'focus_uid': focus_uid}, function (data) {
                if (data.code == 1) {
                    $("#focus_uid span").attr('class', 'follow-me').html('加关注');
                    $("#focus_uid i").attr('class','fa fa-plus text-size14 cor-grayac');
                }
            })
        }
    });
...
($("#liMit").keyup(function(){
    var oLimit = $('#liMit');
    var oTextCount = $('#textCount');
    var curLength = oLimit.val().length;
    if( curLength > 500 )
    {
        var num = oLimit.val().substr(0,500);
        oLimit.val(num);
    }
    else
    {
        oTextCount.text(500-curLength);
    }
}));
$('.workin-validform').Validform({
    tiptype:3,
});
