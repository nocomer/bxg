...
<div class="taskDetails taskbg clearfix employ-bounty g-getpay">
    <div class="g-recharge"><h4 class="cor-blue u-title">购买增值服务</h4></div>
    <div class="cor-gray51 text-size14">
        <div class="space"></div>
        <p>支付金额：<span class="text-size26 cor-orange text-blod">￥{{$service_cash}}</span></p>
        <div class="space"></div>
        <p>请选择支付方式</p>
    </div>
    <div class="space"></div>
    <div class="tabbable">
        ...
        <div class="tab-content clearfix f-tab">
            ...
            <div class="tab-pane in active clearfix text-size14 cor-gray51 u-pay" id="home1">
                <form method="post" action="{{url('user/serviceBountyPay')}}">
                    {!! csrf_field() !!}
                    <input name="goods_id" type="hidden" value="{!! $good_id !!}">
                    <input name="service_id" type="hidden" value="{{ $service_id }}">
                    <input type="hidden" name="pay_canel" id="pay-canel" value=0 />
                    @if($balance >= $service_cash)
                        <span class="help-block cor-gray51">您的账户可用余额：<span class="cor-orange">￥{{$balance}}</span></span>
                        ...
                        <div class="text-center clearfix">
                            <button type="submit" class="btn btn-primary btn-blue btn-big1 bor-radius2" >确认支付</button>
                            <a href="{{ URL('user/serviceEdit',['id'=>$good_id]) }}" class="cor-gray93 btn-big">返回</a>
                        </div>
                        @else
                                <!--余额不足状态-->
                        <div class="text-size14 cor-gray51">
                            <p>您的账户可用余额：<span class=" cor-orange">￥{{$balance}}</span></p>
                            <button type="button" class="close" data-dismiss="alert">
                                <i class="icon-remove"></i>
                            </button>
                            您的余额不足，请选择其他支付方式或是去　<a href="/finance/cash" class="btn btn-sm btn-orange34 bor-radius2">充值</a>
                            <a href="{{ URL('user/serviceEdit',['id'=>$service_id]) }}" class="btn btn-white bor-radius2">返回</a>
                            <br>
                        </div>
                    @endif
                </form>
            ...
            <div class="tab-pane clearfix" id="home2">
                <form action="{{url('user/serviceBountyPay')}}" method="post" target="_blank">
                    {{ csrf_field() }}
                    <input name="goods_id" type="hidden" value="{!! $good_id !!}">
                    <input name="service_id" type="hidden" value="{{ $service_id }}">
                ...
                <div class="radio clearfix">
                    @if(isset($pay_config['alipay']))
                        <label class="clearfix inline check-boxItm">
                            <input type="radio" name="pay_type" value='1' class="ace" checked="checked"/>
                ...
                <div class="text-center clearfix">
                    ...
                    <a href="{{ URL('user/serviceEdit',['id'=>$service_id]) }}" class="cor-gray93 btn-big">返回</a>
                </div>
                <div class="space"></div>
                </form>
            </div>
                 ...
                 <div class="modal-body text-center clearfix">
                     ...
                     <div class="col-sm-8 hidden-xs">
                         <div class="cor-gray51 text-left">
                             ...
                             <div class="space"></div>
                             <p>
                                 <a href="{{ URL('shop/buyservice',['id'=>$service_id]) }}" type="button" class="btn btn-primary btn-sm btn-blue btn-big1 bor-radius2" >已完成付款</a>&nbsp;
                                 <a href="{{ URL('user/serviceList') }}" class="cor-gray97 modaltxt">支付遇到问题</a>
                             </p>
                             <p><a href="{{ URL('/user/serviceBounty',['id'=>$service_id]) }}" class="cor-blue167 text-under">返回选择其他支付方式></a></p>
                         </div>
                     </div>
                     ...
                     <div class="hidden-lg hidden-md hidden-sm visible-xs-12">
                         <div class="cor-gray51 text-center">
                             ...
                             <p><a href="{{ URL('user/serviceBounty',['id'=>$service_id]) }}" class="cor-blue167" data-dismiss="modal">返回选择其他支付方式></a></p>
                         </div>
                     </div>
                 </div>
             </div>
