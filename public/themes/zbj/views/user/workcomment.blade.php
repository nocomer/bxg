                    ...
                    <div class="col-sm-1 col-xs-3">
                        <div class="row">
                            @if($v['type']==1)
                                ...
                                <div class="col-sm-11 col-xs-9 s-myborder">
                                    <div class="clearfix">
                                        <span class=" pull-left text-muted text-size12 cor-gray87 s-myname">任务：<a  class="cor-blue42" target="_blank" href="{{ url('task',['id'=>$v['task_id']]) }}">  {{ $v['title'] }}</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;成交价：￥{{ $v['bounty'] }}</span>
                                        <a class="pull-right cor-gray87 text-size12" href="javascript:;">{{ date('Y-m-d',strtotime($v['task_create'])) }}</a>
                                    </div>
                                    <div class="space-6"></div>
                                    <div class="p-space">
                        ...
                        <div class="clearfix">
                            {{--<ul class="pagination pull-right">--}}
                                {{--@if(!is_null($comment['prev_page_url']))--}}
                                    {{--<li><a href="{!! URL('user/myCommentOwner').'?'.http_build_query(array_merge($merge,['page'=>$comment['current_page']-1])) !!}">&lt;</a></li>--}}
                                {{--@endif--}}
                                {{--@if($comment['last_page']>1)--}}
                                    {{--@for($i=1;$i<=$comment['last_page'];$i++)--}}
                                        {{--<li class="{{ ($i==$comment['current_page'])?'active':'' }}"><a href="{!! URL('user/myCommentOwner').'?'.http_build_query(array_merge($merge,['page'=>$i])) !!}" class="bg-blue">{{ $i }}</a></li>--}}
                                    {{--@endfor--}}
                                {{--@endif--}}
                                {{--@if(!is_null($comment['next_page_url']))--}}
                                    {{--<li><a href="{!! URL('user/myCommentOwner').'?'.http_build_query(array_merge($merge,['page'=>$comment['current_page']+1])) !!}">&gt;</a></li>--}}
                                {{--@endif--}}
                            {{--</ul>--}}
                            <div class="pull-right">
                                {!! $comment_page->appends($_GET)->render() !!}
                            </div>
                        </div>
                    </div>
                    @else
