<div class="col-xs-12 col-left">
    <div class="taskDetails alert taskbg clearfix employ-bounty">
        <div class="page-header">
            <h4 class="text-size22 cor-gray51"><strong>您的雇佣提交成功，请尽快托管赏金！</strong></h4>
        </div>
        <div class="cor-gray51 text-size14">
            <div class="space"></div>
            <p>支付金额：<span class="text-size26 cor-orange text-blod">{{ sprintf(" %1\$.2f",$employ_data['bounty']) }}</span></p>
            <div class="space"></div>
            <p>请选择支付方式</p>
        </div>
        <div class="space"></div>
        <div class="tabbable">
            ...
            <div class="tab-content clearfix f-tab">
                ...
                <div class="tab-pane in active clearfix text-size14 cor-gray51 u-pay" id="home1">
                    @if($user_data['balance'] >= $employ_data['bounty'])
                    <form class="form-horizontal" role="form" action="{{ URL('employ/bounty') }}" method="post"  name="bounty-form">
                        {{ csrf_field() }}
                        <input type="hidden" name="id" value="{{ $employ_data['id'] }}" />
                        <input type="hidden" name="pay_canel" id="pay-canel" value=0 />
                        <span class="help-block cor-gray51">您的账户可用余额：<span class="cor-orange">￥{{ $user_data['balance'] }}</span></span>
                        ...
                        <div class="text-center clearfix">
                            ...
                            <a href="{{ URL('employ/create/'.$employ_data['employee_uid']) }}" class="cor-gray93 btn-big">返回</a>
                        </div>
                    </form>
                    @else
                    <!--余额不足状态-->
                   <div class="text-size14 cor-gray51">
                        <p>您的账户可用余额：<span class=" cor-orange">{{ $user_data['balance'] }}</span></p>
                        <button type="button" class="close" data-dismiss="alert">
                            <i class="icon-remove"></i>
                        </button>
                        您的余额不足，请选择其他支付方式或是去　<a href="/finance/cash" target="_blank" class="btn btn-sm btn-orange34 bor-radius2">充值</a>
                        <br>
                    </div>
                    @endif
                </div>
                ...
                <div class="tab-pane clearfix" id="home2">
                    <form class="form-horizontal" role="form" action="{{ URL('employ/bounty') }}" method="post"  name="bounty-form" target="_blank">
                        {{ csrf_field() }}
                        <input type="hidden" name="id" value="{{ $employ_data['id'] }}" />
                        <input type="hidden" name="pay_canel" value=1 />
                        <div class="space"></div>
                    ...
                    <div class="modal-body text-center clearfix">
                        ...
                        <div class="col-sm-8 hidden-xs">
                            <div class="cor-gray51 text-left">
                                ...
                                <div class="space"></div>
                                <p>
                                    <a href="{{ URL('employ/employCheck',['id'=>$employ_data['id']]) }}" type="button" class="btn btn-primary btn-sm btn-blue btn-big1 bor-radius2" >已完成付款</a>&nbsp;
                                    <a href="{{ URL('employ/bounty',['id'=>$employ_data['id']]) }}" class="cor-blue167 text-under">支付遇到问题</a>
                                </p>
                                <p><a href="{{ URL('employ/bounty',['id'=>$employ_data['id']]) }}" class="cor-blue167 text-under">返回选择其他支付方式></a></p>
                            </div>
                        </div>
