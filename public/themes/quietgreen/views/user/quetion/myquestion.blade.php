        ...
        <form action="{{ URL('user/myquestion') }}" method="get" id="screen_form">
            <div class="control-group pull-left">
                <select class="form-control" name="type">
                    ...
                    <option value="1" class="ace screen" @if(isset($_GET['type']) && $_GET['type'] == 1) selected="selected" @endif>等待处理</option>
                    <option value="2" class="ace screen" @if(isset($_GET['type']) && $_GET['type'] == 2) selected="selected" @endif>审核失败</option>
                    <option value="3" class="ace screen" @if(isset($_GET['type']) && $_GET['type'] == 3) selected="selected" @endif>未解决</option>
                    <option value="4" class="ace screen" @if(isset($_GET['type']) && $_GET['type'] == 4) selected="selected" @endif>已解决</option>
                </select>
            </div>
        </form>
    </div>
            ...
            <li class="row width590">
                ...
                <div class="col-sm-11 col-xs-10 usernopd">
                    <div class="col-sm-9 col-xs-8">
                        <div class="text-size14 cor-gray51 p-space"><a class="cor-blue42 p-space" target="_blank"  href="{{ URL('question/check',['id'=>$v['id']]) }}">{{ $v['discription'] }}</a></div>
                        <div class="space-6"></div>
                    ...
                    <div class="col-sm-3 col-xs-4 text-right hiden590"><a class="btn-big bg-blue bor-radius2 hov-blue1b" target="_blank" href="{{ URL('question/check',['id'=>$v['id']]) }}">查看</a></div>
