                                            ...
                                            {{ (!empty(old('xuanshang')))?'in':'' }} @if($task['type_id'] == $type_v['id'])in active @endif" id="ck">
                                                            <div class="mission-ck z-check-validform-ck">
                                                                ...
                                                                <div>
                                                                    <input type="text" name="bounty{{$type_v['alias']}}" id="bounty"
                                                                           @if($task['type_id'] == $type_v['id'])value="{{$task['bounty']}}" @endif  class="mis-txt"
                                                                ...
                                                                <div>
                                                                    <input type="text" name="worker_num{{$type_v['alias']}}" class="mis-txt"
                                                                           datatype="positive"  nullmsg="请填写服务商数量！" @if($task['type_id'] == $type_v['id'])value="{{$task['worker_num']}}" @endif
                                                                           errormsg="请填写大于0的整数！">
                                                        ...
                                                        <span class="input-group-addon date-icon" >
                                                            <i class="fa fa-calendar bigger-110"></i>
                                                                    ...
                                                                    <div>
                                                                        <input type="text" class="input-sm form-control " id="datepiker-begin" onchange="beginAt($(this))" @if($task['type_id'] == $type_v['id']) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif placeholder="开始时间" >
                                                                    </div>
                                                        <span class="input-group-addon  date-icon ">
                                                            <i class="fa fa-exchange"></i>
                                                                ...
                                                                @endif
                                                                <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at"  datatype="*" nullmsg="请填写开始时间！" @if($task['type_id'] == $type_v['id']) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif/>
                                                    ...
                                                    <span class="lbl text-blod text-size16 pull-left">
                                                        ...
                                                        <span class="input-group-addon date-icon" >
                                                            <i class="fa fa-calendar bigger-110"></i>
                                                                    ...
                                                                    <div>
                                                                        <input type="text" class="input-sm form-control " id="datepiker-begin1" onchange="beginAt($(this))" @if($task['type_id'] == $type_v['id']) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif
                                                                               placeholder="开始时间" >
                                                                    </div>
                                                        <span class="input-group-addon  date-icon ">
                                                            <i class="fa fa-exchange"></i>
                                                                ...
                                                                @endif
                                                                <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at1" datatype="*" nullmsg="请填写开始时间！" @if($task['type_id'] == $type_v['id']) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif class="begin_at"/>
                                                                <input type="hidden" id="delivery_deadline1"
name="delivery_deadline{{$type_v['alias']}}" ajaxurl="/task/checkDeadlineByBid"  datatype="*" nullmsg="请填写截止时间！" @if($task['type_id'] == $type_v['id']) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif class="delivery_deadline"/>
                                                    <span class="time_check validform_checktip"
                                                          style="width:90%;margin-left:20px;display:none">
                                                                ...
                                                                <div>
                                                                    <input type="text" name="bounty{{$type_v['alias']}}" class="mis-txt" @if($task['type_id'] == $type_v['id'])value="{{$task['bounty']}}" @endif placeholder="若无明确预算，可不填">
                                                                </div>
                                                                <ul class="clearfix checkbox process">
                                                                    <li class="pull-left">
                                                                        <span></span>
                            ...
                            <span>￥
                                ...
                                <button class="btn btn-primary btn-blue bor-radius2 btn-big1 preservation" onclick="sluSub(1)">保存</button><a href="javascript:sluSub(2);"  class="text-size14 preview">预览任务</a><a href="javascript:sluSub(3);" class="text-size14 not_releasedsss">暂不发布</a>
                            </div>
                        </div>
                    </form>
                </div>
