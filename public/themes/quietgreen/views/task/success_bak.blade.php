    ...
    <div class="row  case-list">
        @foreach($list as $v)
            <div class="col-md-3 col-xs-12 col-sm-4 case-list-item">
                <a class="case-list-item-img" href="@if(Auth::check() && Auth::user()->id == $v['uid']) {!! '/user/personevaluationdetail/'.$v['id']  !!} @elseif( !empty($v['url'])) {{  $v['url'] }}  @else {!! '/task/successDetail/'.$v['id']  !!} @endif" target="_blank"><img src="{{ $domain.'/'.$v['pic'] }}" alt=""></a>
                <div class="case-list-item-name">
                    <p><a href="@if(Auth::check() && Auth::user()->id == $v['uid']) {!! '/user/personevaluationdetail/'.$v['id']  !!} @elseif( !empty($v['url'])) {!! $v['url'] !!}  @else {!! '/task/successDetail/'.$v['id']  !!} @endif" target="_blank">{{ $v['title'] }}</a></p>
