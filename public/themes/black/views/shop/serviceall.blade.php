<article class="col-xs-12 col-left ">
	<div class="successstory">
		<h4 class="text-size20 tit mg-margin0">所有服务</h4>
		@if(!empty($cateInfo))
	    <div class="alink">
			@foreach($cateInfo as $cv)
	        <a class="cor-gray45" href="{!! URL('/shop/serviceAll/'.$shopId.'?cate_id='.$cv['cate_id']) !!}">{!! $cv['name'] !!} <span>({!! $cv['num'] !!})</span></a>
			@endforeach　　
	    </div>
		@endif
	    <div class="shop-mainlistwrap">
			@if($serviceInfo->total())
	    	<ul class="row shop-mainlist successstory-list">
				@foreach($serviceInfo as $v)
		        <li class="col-md-3 col-sm-4 col-xs-6">
		            <div class="shop-mainimg shop-mainimg168">
						@if($v->cover)
						<a href="{!! URL('shop/buyservice/'.$v->id) !!}"><img src="{!! $domain.'/'.$v->cover !!}" alt="" onerror="onerrorImage('{{ Theme::asset()->url('images/employ/bg2.jpg')}}',$(this))"></a>
						@else
						<a href="{!! URL('shop/buyservice/'.$v->id) !!}"><img src="{{ Theme::asset()->url('images/case_back.png') }}" alt=""></a>
						@endif
					</div>
		            <div class="shop-maininfo">
		                <h5 class="text-size14 p-space"><a class="cor-gray51" href="{!! URL('shop/buyservice/'.$v->id) !!}">{!! $v->title !!}</a></h5>
			...
			<div class="row close-space-tip">
				<div class="col-md-12 text-center">
					...
					<p class="text-size16 cor-gray87">暂无服务</p>
