...
<form class="form-horizontal clearfix registerform" role="form" action="{!! url('manage/editInterview/'.$id) !!}" method="post">
    {!! csrf_field() !!}
    <div class="g-backrealdetails clearfix bor-border">
        <div class="bankAuth-bottom clearfix col-xs-12">
            <p class="col-sm-1 control-label no-padding-left" for="form-field-1" > 访谈标题：</p>
            <p class="col-sm-4">
                <input type="text" id="form-field-1"  class="col-xs-10 col-sm-5" value="{{$interview['title']}}" name="title">
                {!! $errors->first('title') !!}
            </p>
        </div>
        <div class="bankAuth-bottom clearfix col-xs-12">
            <p class="col-sm-1 control-label no-padding-left" for="form-field-1"> 访谈店主：</p>
            <p class="col-sm-4">
                <select name="shop_id">
                    <option value="0" >请选择</option>
                    @if(!empty($shopInfo))
                        @foreach($shopInfo as $sv)
                            <option value="{!! $sv['id'] !!}" @if($interview['shop_id'] == $sv['id']) selected="selected" @endif >{!! $sv['shop_name'] !!}</option>
                        @endforeach
                    @endif
                </select>
        ...
        <div class="bankAuth-bottom clearfix col-xs-12">
            ...
            <p class="col-sm-4">
                <textarea name="desc" id="" cols="30" rows="10" >{{$interview['desc']}}</textarea>
                {!! $errors->first('desc') !!}
            </p>
        </div>
        <div class="bankAuth-bottom clearfix col-xs-12">
            ...
            <p class="col-sm-4">
                <input type="text" id="form-field-1"  class="col-xs-10 col-sm-5"  name="list" datatype="*" value="{{$interview['list']}}">
                {!! $errors->first('list') !!}
            </p>
        </div>
        <div class="col-xs-12">
