...
(function ($, undefined) {
    ...
    Tree.prototype = {
        constructor: Tree, render: function () {
            ...
            this.options.dataSource.data($el.data(), function (items) {
                ...
                $.each(items.data, function (index, value) {
                    ...
                    if (value.type === "folder") {
                        ...
                        header.data(value);
                        if ("icon-class" in value)header.find("i").addClass(value["icon-class"]);
            ...
            $el.trigger("updated", {info: data, item: $el, eventType: eventType})
        }, selectFolder: function (el) {
            var $el = $(el);
            var $parent = $el.parent();
            var $treeFolderContent = $parent.find(".tree-folder-content");
            var $treeFolderContentFirstChild = $treeFolderContent.eq(0);
            var target = "." + $.trim(this.options["close-icon"].replace(/\s/g, "."));
            var eventType, classToTarget, classToAdd;
            if ($el.find(target).length) {
                eventType = "opened";
                classToTarget = this.options["close-icon"];
                classToAdd = this.options["open-icon"];
                $treeFolderContentFirstChild.show();
                if (!$treeFolderContent.children().length) {
                    this.populate($el)
                }
            } else {
                target = "." + $.trim(this.options["open-icon"].replace(/\s/g, "."));
                eventType = "closed";
            ...
            $parent.find(target).eq(0).removeClass(classToTarget).addClass(classToAdd);
            ...
            this.$element.find(".icon-folder-open").each(function () {
                var $this = $(this).removeClass("icon-folder-close icon-folder-open").addClass("icon-folder-close");
    ...
    $.fn.tree.defaults = {multiSelect: false, loadingHTML: "<div>Loading...</div>", cacheItems: true};
