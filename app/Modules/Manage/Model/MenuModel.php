...
class MenuModel extends Model
{
    ...
    static public function getMenuPermission()
    {
        ...
        if($uid==1)
        {
            ...
            $manageMenuAll = self::all()->toArray();//所有菜单
            $third_menu = self::whereIn('id',$menu_ids)->where('level',3)->lists('id')->toArray();//三级权限所有
            $second_menu = self::whereIn('id',$menu_ids)->where('level',2)->lists('id')->toArray();//二级权限所有
            $first_menu = self::whereIn('id',$menu_ids)->where('level',1)->lists('id')->toArray();//一级权限所有
